...
/***********************************/
  ...
  // back and forth.
	...
	printf("\n ************ MODE Enviar DADES ***********\n");
		...
		liniatext = node + "_" + ordre;

                std::cout << "Enviem el seguent: " << liniatext << "\n"; // això equival a un printf per a Strings

		strncpy(info_rebuda, liniatext.c_str(), sizeof(info_rebuda));
		
		bool ok = 0;
		ok = radio.write( &info_rebuda, sizeof(info_rebuda) );
		
		if (!ok){ // Si no s'ha pogut enviar. Per anar bé, ho hauriem de reintentar.
			printf("failed.\n");
		}
		else
		{
